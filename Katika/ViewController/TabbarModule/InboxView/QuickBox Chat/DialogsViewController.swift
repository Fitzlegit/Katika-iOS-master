...
class DialogTableViewCellModel: NSObject {
    ...
    init(dialog: QBChatDialog) {
        ...
        if dialog.type == .private {
            self.dialogIcon = UIImage(named: "img_UserQuickBox")
        }
        else {
...
class DialogsViewController: UITableViewController, QMChatServiceDelegate, QMChatConnectionDelegate, QMAuthServiceDelegate {
    ...
    func createLogoutButton() -> UIBarButtonItem {
        ...
        let logoutButton = UIBarButtonItem(title: "SA_STR_LOGOUT".localized, style: UIBarButtonItem.Style.plain, target: self, action: #selector(DialogsViewController.logoutAction))
        return logoutButton
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var str_CellIdentifier : String = "nodata"
        if dialogs()?.count != 0 {
            str_CellIdentifier = "dialogcell"
        }
        
        
        if dialogs()?.count != 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: str_CellIdentifier, for: indexPath) as! DialogTableViewCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
         ...
         if dialogs()?.count != 0 {
            ...
            self.performSegue(withIdentifier: "goToChat".localized , sender: dialog)
        }
    }
