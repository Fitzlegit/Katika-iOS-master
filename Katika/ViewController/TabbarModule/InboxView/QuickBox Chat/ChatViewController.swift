...
//        QMMessageNotificationManager.showNotification(withTitle: title, subtitle: subtitle, type: .error)
        ...
        //Alert show for Header
        messageBar.MessageShow(title:title as NSString, alertType: MessageView.Layout.CardView, alertTheme: .error, TopBottom: true)
    }
    ...
    override func attributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.font)] = UIFont(name: "Helvetica", size: 17)
        
        let attributedString = NSAttributedString(string: messageItem.text!, attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes))
    ...
    override func topLabelAttributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString? {
        ...
        var attributes = Dictionary<String, AnyObject>()
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor)] = UIColor(red: 11.0/255.0, green: 96.0/255.0, blue: 255.0/255.0, alpha: 1.0)
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.font)] = UIFont(name: "Helvetica", size: 17)
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle)] = paragrpahStyle
        ...
        if let topLabelText = ServicesManager.instance().usersService.usersMemoryStorage.user(withID: messageItem.senderID)?.login {
            topLabelAttributedString = NSAttributedString(string: topLabelText, attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes))
        } else { // no user in memory storage
            topLabelAttributedString = NSAttributedString(string: "@\(messageItem.senderID)", attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes))
        }
    ...
    override func bottomLabelAttributedString(forItem messageItem: QBChatMessage!) -> NSAttributedString! {
        ...
        var attributes = Dictionary<String, AnyObject>()
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor)] = textColor
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.font)] = UIFont(name: "Helvetica", size: 13)
        attributes[convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle)] = paragrpahStyle
        ...
        let bottomLabelAttributedString = NSAttributedString(string: text, attributes: convertToOptionalNSAttributedStringKeyDictionary(attributes))
